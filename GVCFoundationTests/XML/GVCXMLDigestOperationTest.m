...
- (void)testParse
{
    ...
    [xml_op setDidFinishBlock:^(GVCOperation *operation) {
        ...
        XCTAssertTrue([[feed feedEntries] count] == 47, @"Feed entries count %lu", (unsigned long)[[feed feedEntries] count]);
...
- (void)testDownloadAndParse
{
	...
	[url_Op setDidFinishBlock:^(GVCOperation *operation) {
        ...
        [xml_op setDidFinishBlock:^(GVCOperation *xoperation) {
            ...
            XCTAssertTrue([[feed feedEntries] count] == 100, @"Feed entries count %lu", (unsigned long)[[feed feedEntries] count]);
